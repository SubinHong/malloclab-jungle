...
// /* segrated의 주소들을 담는 곳의 첫번째를 가리키는 포인터 */
// static char* seg_listp;

// /* segregated list를 만드는 함수 */
// void seg_init(void) {
// /* segregated list 만들 공간 할당 */
// 	if ((seg_listp = mem_sbrk(32 * WSIZE)) == (void*)-1) return;

// /* 2^0 부터 2^32 까지 NULL값으로 초기화 */
// 	for (int i = 0; i < 32; i++) {										 
// 		PUT_P(seg_listp + (i * WSIZE), NULL);
// 	}
// }

// /* size에 맞는 segregated point 찾는 함수 */
// static void* seg_find(int size) {
// 	static char* seg;

// 	int i = 0;
// /* 2^32 부터 비트연산으로 적당한 값을 찾음 */
// 	for (i = 32; i > 0; i--) {				 							 
// 		if ((size & (1 << (i))) > 0) {
// 			break;
// 		}
// 	}
// /* 2^n 에 맞는 n 번째 segretated 주소 반환 */
...
// 	int i = 0;
// /* 2^32 부터 비트연산으로 적당한 값을 찾음 (작은값은 필요없다.) */
// 	for (i = 32; i > 0; i--) {
// 		if ((asize & (1 << (i))) > 0) {
// 			break;
// 		}
...
// /* best 값을 비교하기 위한 초기값 */
// 			best = (1 << (j + 1));
// /* segregated list부터 찾아서 들어감 */
// 			for (bp = PREV_FREE_BLKP(seg); bp != (char*)NULL; bp = PREV_FREE_BLKP(bp)) {
// /* block이 주어진 사이즈보다 fit하고 best라면 */
...
// /* 그 free 블록에 나(bp)를 연결함 */
// 		CHANGE_NEXT(PREV_FREE_BLKP(seg), bp);
// 	}
// /* 나의 이전은 기존의 root에 연결되 있던 블록 */
...
// /* 만약 뒤에 free 블록이 있고, 지금과 합쳤을 때 필요한 size를 담을 수 있다면 */
// 	if (!GET_ALLOC(HDRP(NEXT_BLKP(old_dp))) && old_size + old_next_size >= size) {
// 		size_t asize;


// /* 만약 입력받은 사이즈가 0 이면 무시함 */
// 		if (size == 0)
// 			return NULL;
